#!/usr/bin/env bash
# Refuse outgoing commits with automated attribution or agent authorship.
set -euo pipefail
...
for commit in $commits; do
  ...
  if ! bash "$script_dir/check-author.sh" \
    "$(git log -1 --format='%an <%ae>' "$commit")" >/dev/null; then
